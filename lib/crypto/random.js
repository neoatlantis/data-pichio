...
function randomBytes(length){
    ...
    // from `process.hrtime`.
    var hrtimeCollected = [];
    for(var i=0; i<2000 + length * 10; i++) 
    ...
    pool.push(new $.node.buffer.Buffer(JSON.stringify(process.env)));
    ...
    return $.crypto.hash.PBKDF2(
        ...
        8, // should be randomly enough. We aren't deriving encrypting key.
        length
    );
};
