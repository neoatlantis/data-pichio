...
function _SYMCRYPT(){
    ...
    var encrypt = function(dataBuffer, CB){
        ...
        $.node.async.waterfall([
            ...
            function(raw, RR){
                // cascade encryption
                var ciphertext = raw;
                ciphertext = GibberishAESEncrypt(keys[0], ciphertext);
                ciphertext = OpenSSLEncrypt('bf-cbc', keys[1], ciphertext);
                ciphertext = OpenSSLEncrypt('cast5-cbc', keys[2], ciphertext);
                ciphertext = OpenSSLEncrypt('camellia-256-cbc', keys[3], ciphertext);
    ...
    var decrypt = function(dataBuffer, CB){
        ...
        $.node.async.waterfall([
            function(RR){
                ...
                plaintext = OpenSSLDecrypt('camellia-256-cbc', keys[3], plaintext);
                plaintext = OpenSSLDecrypt('cast5-cbc', keys[2], plaintext);
                plaintext = OpenSSLDecrypt('bf-cbc', keys[1], plaintext);
                plaintext = GibberishAESDecrypt(keys[0], plaintext);
    ...
    this.version = function(){
        return 'NCSC512'; // NeoAtlantis Cascade Symmetric Cipher - 512 bit
    };
